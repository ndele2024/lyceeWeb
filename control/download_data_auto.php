...
$xml=new XMLWriter();
$xml->openUri("../data/donneesWebSygbuss$matetab.xml");
$id=1;
$xml->setIndent(true);
...
$xml->writeAttribute("id", $id);
$xml->writeElement("matriculeetab", $trimestre);
$xml->writeElement("nomannee", $annesco);
$xml->endElement();
...
$xml->writeAttribute("id", $id);
$tabeval=Evaluation::getAllEvaluation($annesco, $matetab);
$id++;
...
$xml->writeAttribute("id", $id);
$tabcomp=Evaluation::getAllDonneeCompetence($annesco, $matetab);
$id++;
...
$xml->writeAttribute("id", $id);
$tabstatfait=Statfait::getAllStatfait($annesco, $matetab);
$id++;
...
$xml->writeAttribute("id", $id);
$tabstatprevu=StatPrevue::getAllStatPrevue($annesco, $matetab);
$id++;
...
$xml->writeAttribute("id", $id);
$tabperso=Teacher::getAllEmailprof($matetab);
$id++;
...
$xml->writeAttribute("id", $id);
$tabAbsence = Absence::getAllDonneeAbsence($annesco, $trimestre, $matetab);
$id++;
...
$inputFile = "../data/donneesWebSygbuss$matetab.xml";
$outputFile = "../data/donneesWebSygbuss$matetab.ly";
crypte($inputFile, $outputFile);

//header("location:telecharger_donnee_web.php");

if(file_exists("../data/donneesWebSygbuss$matetab.ly")) {
    if(isset($_GET["automate"])){
        $idf=fopen("../data/automate.2lycee.syg", "w+");
        header("location:../data/automate.2lycee.syg");
    }
    else{
        echo "donneesWebSygbuss$matetab.ly";
    }  
}
else {
    echo"echec";
